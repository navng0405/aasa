...
Aasa is a voice-first, local-first safety companion for elders living independently. It runs Gemma 4 on the phone through Google AI Edge LiteRT-LM when available, uses deterministic safety overrides before tool dispatch, stores personal context in on-device Room, and always asks the elder to confirm before launching a call, SMS, or safety action.

This README is the main project overview and demo guide. It records what is implemented, what is intentionally hackathon-grade, and how to demo the product without overclaiming.

## Gemma 4 Good Hackathon Fit

Aasa is strongest for the Health & Sciences and Safety & Trust impact areas, with a clear Special Technology fit for Google AI Edge LiteRT. LiteRT-LM is the main Android runtime for on-device Gemma 4. The optional FastAPI/Ollama bridge is used for local development, prompt iteration, and fallback demos.
